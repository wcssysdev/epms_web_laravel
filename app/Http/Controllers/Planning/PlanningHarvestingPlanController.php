...
/**
 * Planning Harvesting Plan Palm (Estate Manager role).
 * 
 * Harvesting Plan = rencana panen sawit harian per division/block.
 ...
 */
class PlanningHarvestingPlanController extends BaseController
{
    public function index(Request $request)
    {
        [$date] = $this->resolveDate($request, 'planning.harvesting_plan.date', '+1 day');
    ...
    public function detail(Request $request)
    {
        ...
        if (!$division || !$createdBy) {
            return redirect()->route('planning.harvesting_plan.index', ['date' => $date])
                ->with('error', 'Missing division or created_by parameter.');
        }

        $plans = DB::table('t_harvesting_plan')
        ...
        if ($plans->isEmpty()) {
            return redirect()->route('planning.harvesting_plan.index', ['date' => $date])
                ->with('error', 'No harvesting plans found for selected criteria.');
        }

        return view('planning.harvesting_plan.detail', [
            'title' => 'Harvesting Plan Detail',
            'date' => $date,
            'division' => $division,
            'plans' => $plans,
        ]);
    ...
    public function submitForApproval(Request $request)
    {
        ...
        $updated = DB::table('t_harvesting_plan')
        ...
        if ($updated === 0) {
            return redirect()->route('planning.harvesting_plan.index', ['date' => $date])
                ->with('error', 'No draft harvesting plans found to submit.');
        }

        return redirect()->route('planning.harvesting_plan.index', ['date' => $date])
            ->with('success', "$updated harvesting plan(s) submitted for approval.");
    }
