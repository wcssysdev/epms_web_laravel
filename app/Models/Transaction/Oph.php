...
/**
 * OPH (Oil Palm Harvest / Output per harvester) transaction.
 * Approved by Estate Manager (role 2) or Assistant Manager (role 3, division-scoped).
 *
 * Boolean is_approved schema:
...
class Oph extends Model
{
    ...
    protected $table = 't_oph';

    public $incrementing = false;
    protected $keyType   = 'string';

    protected $fillable = [
        'id', 'company_id', 'oph_card_id', 'harvest_method',
        'estate_code', 'plant_code', 'division_code', 'block_code', 'tph_code', 'platform_no',
        'lat', 'long', 'photo', 'notes',
        'mandor_employee_code', 'mandor_employee_name',
        'kerani_panen_employee_code', 'kerani_panen_employee_name', 'customer_code',
        'bunches_total', 'loose_fruits',
        'is_planned', 'is_approved', 'approved_by', 'approved_by_name', 'approved_at',
        'is_restant_permanent', 'is_closed', 'is_deleted', 'integration_status',
        'remark', 'created_by', 'updated_by',
    ];

    protected $casts = [
        'harvest_method'       => 'integer',
        'bunches_total'        => 'integer',
        'loose_fruits'         => 'float',
        'is_planned'           => 'boolean',
        'is_approved'          => 'boolean',
        'is_restant_permanent' => 'boolean',
        'is_closed'            => 'boolean',
        'is_deleted'           => 'boolean',
