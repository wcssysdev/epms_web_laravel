...
@section('content')
<div class="max-w-4xl" x-data="fdnForm()">
    <form method="POST"
        ...
        <div class="rounded-xl border shadow-sm overflow-hidden mb-5"
            ...
            <div class="px-5 py-4 border-b" style="border-color: var(--epms-border);">
                <h2 class="text-sm font-semibold" style="color: var(--epms-text);">Delivery Note Details</h2>
            </div>
            <div class="p-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-5">
                    ...
                    <x-form.select name="deliver_to_code" label="Deliver To (Destination)"
                                   :value="old('deliver_to_code', $item?->deliver_to_code)">
                        <option value="">— None —</option>
                        @foreach($destinations as $dst)
                            <option value="{{ $dst->destination_code }}"
                                @selected(old('deliver_to_code', $item?->deliver_to_code) === $dst->destination_code)>
                    ...
                    <x-form.input name="bin_number" label="BIN Number"
        ...
        <div class="rounded-xl border shadow-sm overflow-hidden mb-5"
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b" style="border-color: var(--epms-border);">
                <h2 class="text-sm font-semibold" style="color: var(--epms-text);">Delivered OPH Lines</h2>
                <div class="flex items-center gap-4 text-sm">
                    ...
                    <span style="color: var(--epms-text-muted);">Bunches: <span class="font-bold text-primary" x-text="totalBunches"></span></span>
                    <button type="button" @click="loadAvailable()"
                            ...
                            style="border-color: var(--epms-border); color: var(--epms-text);">Load Available OPH</button>
                </div>
            </div>
            <div class="p-5">
                <div x-show="available.length > 0" class="mb-4 rounded-lg border p-3" style="border-color: var(--epms-border);">
                    <p class="text-xs font-medium mb-2" style="color: var(--epms-text-muted);">Available OPH for the selected division — click to add:</p>
                    <div class="flex flex-wrap gap-2">
                        <template x-for="o in available" :key="o.id">
                            <button type="button" @click="addDetail(o)"
                                    ...
                                    style="border-color: var(--epms-border); color: var(--epms-text);"
                                    x-text="`${o.oph_card_id ?? o.id} · blk ${o.block_code ?? '-'} · ${o.bunches_total ?? 0}b`"></button>
                        </template>
                    </div>
                </div>
                ...
                <table class="w-full text-sm" style="color: var(--epms-text);">
                    <thead>
                        <tr class="border-b" style="border-color: var(--epms-border);">
                            <th class="px-2 py-2 text-left text-xs uppercase" style="color: var(--epms-text-muted);">OPH / Card</th>
                            <th class="px-2 py-2 text-left text-xs uppercase" style="color: var(--epms-text-muted);">Block</th>
                            <th class="px-2 py-2 text-left text-xs uppercase" style="color: var(--epms-text-muted);">Bunches</th>
                            <th class="px-2 py-2 text-left text-xs uppercase" style="color: var(--epms-text-muted);">Loose Fruit</th>
                    ...
                    <tbody>
                        <template x-for="(d, i) in details" :key="i">
                            <tr class="border-b" style="border-color: var(--epms-border);">
                                <td class="px-2 py-1.5">
                                    ...
                                    <input type="hidden" :name="`details[${i}][oph_card_id]`" :value="d.oph_card_id">
                                    <input type="hidden" :name="`details[${i}][oph_block_code]`" :value="d.oph_block_code">
                                    <input type="hidden" :name="`details[${i}][oph_tph_code]`" :value="d.oph_tph_code">
                                </td>
                                <td class="px-2 py-1.5" x-text="d.oph_block_code || '-'"></td>
                                <td class="px-2 py-1.5">
                                    <input type="number" min="0" :name="`details[${i}][bunches_delivered]`"
                                           x-model="d.bunches_delivered" @input="recalc()"
                                ...
                                <td class="px-2 py-1.5">
                                    <input type="number" step="0.01" min="0" :name="`details[${i}][loose_fruit_delivered]`"
                                           x-model="d.loose_fruit_delivered"
                                           class="w-24 rounded border px-2 py-1 text-sm"
                                           style="background: var(--epms-header-bg); color: var(--epms-text); border-color: var(--epms-border);">
                                </td>
                        ...
                        <tr x-show="details.length === 0">
                            <td colspan="5" class="px-2 py-3 text-center text-xs" style="color: var(--epms-text-muted);">
                                No OPH lines. Choose a division and click "Load Available OPH".
                            </td>
                        </tr>
                    </tbody>
                </table>
...
function fdnForm() {
    return {
        ...
        init() { this.recalc(); },
        recalc() {
            this.totalBunches = this.details.reduce((s, d) => s + (parseInt(d.bunches_delivered || 0, 10) || 0), 0);
        },
        loadAvailable() {
        ...
        addDetail(o) {
            this.details.push({
                ...
                oph_tph_code: o.tph_code,
                bunches_delivered: o.bunches_total ?? 0, loose_fruit_delivered: o.loose_fruits ?? 0,
            });
