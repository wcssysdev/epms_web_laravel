...
@section('content')
<div class="max-w-2xl" x-data="harvesterForm()">
    <div class="rounded-xl border shadow-sm overflow-hidden"
         style="background: var(--epms-header-bg); border-color: var(--epms-border);">
        <div class="px-5 py-4 border-b" style="border-color: var(--epms-border);">
            <h2 class="text-sm font-semibold" style="color: var(--epms-text);">Harvester Assignment Details</h2>
        </div>
        <form method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-5">
                ...
                <x-form.input name="tph_code" label="TPH Code"
                              :value="old('tph_code', $item?->tph_code)"/>

                <x-form.select name="harvester_employee_code" label="Harvester" required
                               :value="old('harvester_employee_code', $item?->harvester_employee_code)">
                    <option value="">— Select Harvester —</option>
                    @foreach($employees as $e)
                        <option value="{{ $e->employee_code }}"
                            @selected(old('harvester_employee_code', $item?->harvester_employee_code) === $e->employee_code)>
