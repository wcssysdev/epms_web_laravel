...
@section('breadcrumb')
    ...
    <li><span class="font-medium text-primary">Goods Receipt</span></li>
@endsection

@section('page-title', 'Goods Receipt (GR)')
@section('page-subtitle', 'Manage material goods receipt transactions')

@section('page-actions')
    <a href="{{ route('transactions.goods_receipt.create') }}"
...
<div x-data="{ showDelete: false, deleteUrl: '' }">
    ...
    @if(!empty($overQtyPoNumbers))
    <div class="flex items-start gap-3 rounded-xl border border-yellow-300 bg-yellow-50 px-4 py-3 mb-5 text-sm text-yellow-800">
        ...
        <div>
            <strong>Warning:</strong> Some PO(s) have received quantity exceeding the PO order quantity:
            <span class="font-mono">{{ implode(', ', $overQtyPoNumbers) }}</span>
        </div>
    </div>
    @endif
    ...
    <form method="GET" class="flex flex-wrap items-end gap-3 rounded-xl border px-5 py-4 mb-5"
        ...
        <div>
            ...
            <input type="date" name="from" value="{{ $from }}"
                   class="rounded-lg border px-3.5 py-2 text-sm outline-none focus:border-primary"
        ...
        <table class="w-full text-sm">
            <thead>
                <tr style="background: var(--epms-border);">
                    ...
                    <th class="px-3 py-3 text-left font-semibold">GR ID</th>
            ...
            <tbody class="divide-y" style="divide-color: var(--epms-border);">
                @forelse($records as $i => $gr)
                <tr class="hover:bg-black/5 transition {{ in_array($gr->po_number, $overQtyPoNumbers) ? 'bg-yellow-50/30' : '' }}">
                    <td class="px-3 py-3" style="color: var(--epms-text-muted);">{{ $i + 1 }}</td>
                    <td class="px-3 py-3 font-mono text-xs font-medium" style="color: var(--epms-text);">{{ $gr->id }}</td>
                    <td class="px-3 py-3" style="color: var(--epms-text);">{{ $gr->gr_date->format('d M Y') }}</td>
                    <td class="px-3 py-3 font-mono text-xs">
                        {{ $gr->po_number }}
                        @if(in_array($gr->po_number, $overQtyPoNumbers))
                        <span class="ml-1 text-yellow-600" title="Qty exceeds PO">⚠</span>
                        @endif
                    </td>
                    <td class="px-3 py-3 text-xs" style="color: var(--epms-text);">{{ $gr->sloc_code }}</td>
                    <td class="px-3 py-3 text-xs" style="color: var(--epms-text-muted);">{{ $gr->details->count() }} item(s)</td>
                    <td class="px-3 py-3 font-mono text-xs" style="color: var(--epms-text);">{{ $gr->gr_document_number ?: '-' }}</td>
                    <td class="px-3 py-3">
                        <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium {{ $gr->statusBadgeClass() }}">
                            {{ $gr->statusLabel() }}
                        </span>
                    </td>
                    <td class="px-3 py-3">
                        <div class="flex gap-1">
                            <a href="{{ route('transactions.goods_receipt.detail', $gr->id) }}" class="btn-action btn-view" title="View">
                            ...
                            @if($gr->isEditable())
                            <a href="{{ route('transactions.goods_receipt.edit', $gr->id) }}" class="btn-action btn-edit" title="Edit">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                </svg>
                            </a>
                            <button @click="deleteUrl = '{{ route('transactions.goods_receipt.destroy', $gr->id) }}'; showDelete = true"
                ...
                @empty
                <tr><td colspan="9" class="px-3 py-10 text-center text-sm" style="color: var(--epms-text-muted);">No Goods Receipt records for selected date range.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div x-show="showDelete" x-cloak class="fixed inset-0 z-50 flex items-center justify-center" x-transition>
        ...
        <div class="relative w-full max-w-sm rounded-xl border p-6 shadow-xl z-10"
            ...
            <p class="text-sm mb-5" style="color: var(--epms-text-muted);">Delete this Goods Receipt record? This cannot be undone.</p>
