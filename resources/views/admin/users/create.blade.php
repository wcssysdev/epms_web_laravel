                ...
                <div class="form-control mb-4">
                    ...
                    <select name="role_id" id="role_id" required @change="onRoleChange($event)"
                        ...
                        @foreach($roles as $role)
                            <option value="{{ $role->id }}" data-code="{{ $role->role_code }}" {{ old('role_id') == $role->id ? 'selected' : '' }}>
                                {{ $role->role_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <select name="scope_estates[]" multiple size="6"
                    ...
                    @foreach($scopeEstates as $e)
                        <option value="{{ $e->id }}" data-company="{{ $e->company_id }}"
                            {{ collect(old('scope_estates', []))->contains($e->id) ? 'selected' : '' }}>
            ...
            <div x-show="wantsCountries" x-cloak class="mb-5">
                ...
                <select name="scope_countries[]" multiple size="4"
                    ...
                    @foreach($scopeCountries as $c)
                        <option value="{{ $c->id }}"
                            {{ collect(old('scope_countries', []))->contains($c->id) ? 'selected' : '' }}>
                            {{ $c->code }} — {{ $c->name }}
                        </option>
                    @endforeach
                </select>
...
function createUserForm() {
    return {
        loading:  false,
        roleId:   @js((string) old('role_id', '')),
        roleCode: @js(old('role_id') ? '' : ''),
        get wantsEstates()   { return this.roleCode === 'pc' || this.roleCode === 'cs'; },
        get wantsCountries() { return this.roleCode === 'country_admin'; },
        init() {
            // Resolve role_code from the currently selected option (handles old() repopulation)
