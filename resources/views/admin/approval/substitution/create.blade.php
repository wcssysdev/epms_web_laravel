...
<div class="portlet light bordered">
    <div class="portlet-title">
        <div class="caption">
            ...
            <span class="caption-subject bold uppercase">Add Manager Substitution</span>
        </div>
        <div class="actions">
            <a href="{{ route('admin.substitution.index') }}" class="btn btn-default">
                    ...
                    <div class="col-md-6">
                        <select name="approval_substitution_employee_code" class="form-control select2" required>
                            ...
                            @foreach($estateManagers as $em)
                                <option value="{{ $em->id }}" {{ old('approval_substitution_employee_code') == $em->id ? 'selected' : '' }}>
                                    {{ $em->name }} ({{ $em->employee_code }})
                                </option>
                            @endforeach
                        </select>
                        <span class="help-block">Person who will be substituted</span>
                    </div>
                </div>
                    ...
                    <div class="col-md-6">
                        <select name="approval_substitution_employee_code_target" class="form-control select2" required>
                            ...
                            @foreach($assistantManagers as $am)
                                <option value="{{ $am->id }}" {{ old('approval_substitution_employee_code_target') == $am->id ? 'selected' : '' }}>
                                    {{ $am->name }} ({{ $am->employee_code }})
                                </option>
                            @endforeach
                        </select>
                        <span class="help-block">Assistant Manager who will substitute</span>
                    </div>
                </div>
