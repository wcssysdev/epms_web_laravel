<header class="sticky top-0 z-30 flex items-center justify-between border-b border-stroke bg-white px-4 py-5 shadow-sm md:px-5 2xl:px-10"
        style="background-color: var(--epms-header-bg); border-color: var(--epms-border);">

    {{-- LEFT: Toggle + Title --}}
    <div class="flex items-center gap-4">
        <button @click="sidebarOpen = !sidebarOpen"
                class="epms-toggle-btn rounded-lg border px-2 py-2 transition-colors">
            <svg width="25" height="24" viewBox="0 0 25 24" fill="currentColor" style="color: var(--epms-text);">
        ...
        <div class="hidden xl:block">
            <h1 class="mb-0.5 text-xl font-bold" style="color: var(--epms-text);">
                EPMS <span class="text-primary uppercase">{{ session('estate_name', 'Estate') }}</span>
            </h1>
            <p class="text-sm font-medium" style="color: var(--epms-text-muted);">
                Electronic Plantation Mobile Solution
                <span class="ml-1 text-xs" style="color: var(--epms-text-muted);">v{{ config('app.version') }}</span>
            </p>
        </div>
    </div>
    ...
    <div class="flex flex-1 items-center justify-end gap-2 min-[375px]:gap-4">
        ...
        <button @click="toggleDark()"
                class="epms-pill-toggle rounded-full p-[5px] outline-none">
            <span class="relative flex gap-2.5">
                {{-- Sliding indicator — position controlled by Alpine --}}
                <span class="epms-pill-indicator absolute size-[38px] rounded-full border transition-all duration-300"
                      :style="isDark ? 'transform:translateX(48px)' : 'transform:translateX(0)'">
                </span>
                {{-- Sun --}}
                <span class="relative z-10 grid size-[38px] place-items-center rounded-full">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="currentColor" style="color: var(--epms-text);">
        ...
        {{-- Notification Bell --}}
        <button class="epms-icon-btn relative grid size-12 place-items-center rounded-full border">
        ...
        <div class="shrink-0" x-data="{ open: false }">
            <div class="relative">
                <button @click="open = !open" @click.outside="open = false" class="rounded align-middle outline-none">
                    <figure class="flex items-center gap-3">
                        ...
                        <figcaption class="flex items-center gap-1 font-medium max-[1024px]:hidden"
                ...
                <div x-show="open"
                     ...
                     class="epms-dropdown absolute right-0 mt-2 w-52 rounded-lg border shadow-lg z-50">
                    <div class="px-4 py-3 border-b" style="border-color: var(--epms-border);">
                        <p class="text-xs truncate" style="color: var(--epms-text-muted);">{{ session('company_name', '') }}</p>
                        <p class="text-sm font-semibold truncate" style="color: var(--epms-text);">{{ session('user_name', '') }}</p>
                    </div>
                    <ul class="py-2">
                        <li>
                            <a href="{{ route('change-password') }}" class="epms-dropdown-item flex items-center gap-2 px-4 py-2 text-sm">
                        ...
                        <li class="border-t mt-1 pt-1" style="border-color: var(--epms-border);">
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-red-500 hover:bg-gray-100">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
