...
@section('title', 'OPH Report')

@section('content')
<div class="container-fluid">
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">OPH (Oil Palm Harvesting) Report</h5>
        </div>
        <div class="card-body">
            <form id="filter-form" class="mb-4">
                <div class="row">
                    ...
                    <div class="col-md-2 d-flex align-items-end">
                        ...
                        <a href="{{ route('reporting.transaction.oph.export') }}" id="btn-export" class="btn btn-success btn-sm">Export</a>
                    </div>
                </div>
            </form>

            <div class="table-responsive">
                <table id="oph-table" class="table table-bordered table-sm">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Date</th>
...
<script>
$(function() {
    let table = $('#oph-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "{{ route('reporting.transaction.oph.index') }}",
    ...
    $('#btn-export').click(function(e) {
        e.preventDefault();
        const params = $.param({from: $('#from').val(), to: $('#to').val(), division: $('#division').val(), block: $('#block').val()});
