@extends('layouts.app')
@section('title', 'Production Detail Report')
@section('content')
<div class="container-fluid">
    <div class="card">
        <div class="card-header"><h5 class="mb-0">Production Detail Report</h5></div>
        <div class="card-body">
            <form id="filter-form" class="mb-4">
                ...
                <div class="row mt-2"><div class="col-md-12"><button type="button" id="btn-filter" class="btn btn-primary">Filter</button><a href="{{ route('reporting.transaction.production-detail.export') }}" id="btn-export" class="btn btn-success">Export CSV</a></div></div>
            </form>
            <div class="table-responsive">
                <table id="production-table" class="table table-bordered table-sm">
                    <thead>
                        <tr>
                            <th>No</th><th>Date</th><th>Division</th><th>Block</th><th>Total Bunches</th><th>Total Loose Fruits</th><th>Delivered Bunches</th><th>Not Delivered</th><th>Delivered LF</th><th>LF %</th>
                        </tr>
                    </thead>
                </table>
            </div>
...
<script>
$(function() {
    let table = $('#production-table').DataTable({
        processing: true, 
        serverSide: true, 
        ajax: {
            url: "{{ route('reporting.transaction.production-detail.index') }}", 
        ...
        columns: [
            ...
            {data: 'block_name'},
            {data: 'total_bunches'},
            {data: 'total_loose_fruits'},
            {data: 'bunches_delivered'},
            {data: 'bunches_not_sent'},
            {data: 'loose_fruit_delivered'},
            {data: 'loose_fruit_percentage'}
        ], 
        pageLength: 25
    });
