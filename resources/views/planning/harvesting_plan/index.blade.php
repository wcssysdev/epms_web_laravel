...
@section('title', 'Harvesting Plan')

@section('breadcrumb')
    <li><span class="text-gray-500">Planning /</span></li>
    <li><span class="font-medium text-primary">Harvesting Plan</span></li>
@endsection

@section('page-title', 'Harvesting Plan')
@section('page-subtitle', 'Plan harvesting targets per block')

@section('page-actions')
    <a href="{{ route('planning.harvesting_plan.create') }}"
...
<div x-data="harvestingPlanPage()">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm" style="color: var(--epms-text);">
                ...
                <tbody>
                    ...
                    <tr class="border-b" style="border-color: var(--epms-border);">
                        ...
                        <td class="px-3 py-3">
                            <div class="flex gap-1">
                                ...
                                @endif
                                @if($p->isEditable())
                                <a href="{{ route('planning.harvesting_plan.edit', $p->id) }}" class="btn-action btn-edit" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    @empty
                    <tr><td colspan="9" class="px-3 py-10 text-center text-sm" style="color: var(--epms-text-muted);">No harvesting plans for this date.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div x-show="showDelete" x-cloak class="fixed inset-0 z-50 flex items-center justify-center" x-transition>
        ...
        <div class="relative w-full max-w-sm rounded-xl border p-6 shadow-xl z-10"
            ...
            <p class="text-sm mb-5" style="color: var(--epms-text-muted);">Delete this harvesting plan?</p>
            <div class="flex gap-3 justify-end">
                ...
                <form :action="`{{ url('planning/harvesting-plan') }}/${deleteId}`" method="POST" class="inline">
                    @csrf @method('DELETE')
...
@push('scripts')
<script>
function harvestingPlanPage() {
    return {
        showApprove: false, approveUrl: '',
        showDelete: false, deleteId: null,
        openApprove(id) {
            this.approveUrl = `{{ url('planning/harvesting-plan') }}/${id}/approve`;
