...
<div class="rounded-xl border shadow-sm overflow-hidden"
    ...
    @if(count($rows) === 0)
        <div class="p-8 text-center text-sm" style="color:var(--epms-text-muted);">
            No coconut harvesting plans for this status.
        </div>
    @else
            ...
            <table class="w-full text-sm" style="color:var(--epms-text);">
                <thead>
                    <tr class="border-b" style="border-color:var(--epms-border);">
                        ...
                        <th class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide"
                            style="color:var(--epms-text-muted);">Total Target (Pcs)</th>
                        <th class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide"
                ...
                <tbody>
                    ...
                    <tr class="border-b hover:opacity-80" style="border-color:var(--epms-border);">
                        ...
                        <td class="px-3 py-2">
                            <a href="{{ route('planning.harvesting_plan_coconut.detail', [
                            ...
                            @if($canSubmit ?? false)
                            <form method="POST" action="{{ route('planning.harvesting_plan_coconut.submit') }}" class="inline ml-2">
                                @csrf
                                ...
                                <button type="submit"
                                        onclick="return confirm('Submit {{ $r['block_count'] ?? 0 }} coconut harvesting plan(s) for approval?')"
