...
@section('page-subtitle', 'Manage daily workplan for divisions')

@section('content')
<div x-data="workplanPage('{{ $date }}')">
    ...
    <div x-show="activeTab==='drafted'" x-cloak>
        @include('planning.workplan._table', ['rows' => $drafted, 'tab' => 'drafted', 'canSubmit' => true])
    </div>

    {{-- SUBMITTED --}}
    <div x-show="activeTab==='submitted'" x-cloak>
        @include('planning.workplan._table', ['rows' => $submitted, 'tab' => 'submitted', 'canSubmit' => false])
    </div>

    {{-- APPROVED --}}
    <div x-show="activeTab==='approved'" x-cloak>
        @include('planning.workplan._table', ['rows' => $approved, 'tab' => 'approved', 'canSubmit' => false])
    </div>

    {{-- REJECTED --}}
    <div x-show="activeTab==='rejected'" x-cloak>
        @include('planning.workplan._table', ['rows' => $rejected, 'tab' => 'rejected', 'canSubmit' => false])
    </div>
