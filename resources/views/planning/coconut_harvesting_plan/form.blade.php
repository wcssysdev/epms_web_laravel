...
@section('title', ($plan ? 'Edit' : 'Add') . ' Coconut Harvesting Plan')

@section('breadcrumb')
    <li><span class="text-gray-500">Planning /</span></li>
    <li><a href="{{ route('planning.coconut_harvesting_plan.index') }}" class="text-gray-500 hover:text-primary">Coconut Harvesting Plan /</a></li>
    <li><span class="font-medium text-primary">{{ $plan ? 'Edit' : 'Add' }}</span></li>
@endsection

@section('page-title', ($plan ? 'Edit' : 'Add') . ' Coconut Harvesting Plan')

@section('page-actions')
    <a href="{{ route('planning.coconut_harvesting_plan.index') }}"
...
@section('content')
<form method="POST"
      action="{{ $plan ? route('planning.coconut_harvesting_plan.update', $plan->id) : route('planning.coconut_harvesting_plan.store') }}"
      x-data="coconutForm()"
      class="max-w-2xl">
    @csrf
        ...
        <div class="p-5 grid gap-x-5 md:grid-cols-2">
            ...
            <div class="form-control mb-4">
                ...
                <select name="block_code" x-model="blockCode" @change="onBlockChange()" required
                    ...
                    <option value="">— Select Coconut Block —</option>
...
function coconutForm() {
    return {
        ...
        async loadBlocks(division) {
            ...
            const res = await fetch(`{{ route('planning.coconut_harvesting_plan.blocks') }}?division_code=${encodeURIComponent(division)}`);
